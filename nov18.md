...
## Full Multi-line Examples – Intermediate Operations
...
**limit()**
```java
List<String> firstTwo = names.stream()\                             .limit(2)
                             .collect(Collectors.toList());
```
...
## Full Multi-line Examples – Terminal Operations
...
**collect()**
```java
List<String> filtered = names.stream()\                             .filter(n -> n.startsWith("A"))
                             .collect(Collectors.toList());
...
Optional<String> any = names.parallelStream()
                            .filter(n -> n.length() > 3)
                            .findAny();
any.ifPresent(System.out::println); // Could be Alice or Charlie
```
...
System.out.println(anyLong); // true
```
...
System.out.println(allStartA); // false
```
...
System.out.println(noneEmpty); // true
```
